...
The USD offers allow `card`, `cashapp`, `crypto`, `us_bank_account`, and `customer_balance`. ACH bank-account verification uses Financial Connections first: both the deferred Payment Element and the server PaymentIntent set `us_bank_account.verification_method` to `automatic`, allowing manual routing/account-number entry and Stripe-hosted microdeposit verification only when instant verification cannot complete. Stripe can use its default one-deposit `SM…` descriptor-code flow or a two-amount `ACCTVERIFY` flow; the integration does not claim to choose between those provider-controlled variants. Financial Connections-linked accounts request all currently documented permissions (`payment_method`, `balances`, `ownership`, and `transactions`) and prefetch balances, ownership, and transactions. Manually linked microdeposit accounts do not provide those Financial Connections data products. Stripe remains the system of record for the connected account and its active/inactive/disconnected status; raw financial data is not exposed through a public SET route or written to application logs. Because bank-transfer customer balances require a Customer, the server creates an idempotent Customer and configures US bank-transfer funding for those offers. The EUR offer allows `card`, `bizum`, `eps`, `mb_way`, and `multibanco`. Cash App Pay is USD-only, and stablecoin payment presentment is USD unless Stripe separately grants private-preview currency support, so neither is included in the EUR allowlist. All three are direct SET billing: they do not use Connect, application fees, transfer fields, Custom Payment Methods, or automatic tax.
...
Then run `npm run dev` and open one of the checkout URLs above. Stripe returns to `/checkout/return` after any required customer action. Financial Connections can verify the bank account during the customer session. A manual fallback remains pending until the customer completes Stripe's microdeposit verification, which ordinarily takes 1–2 business days in live mode. ACH debit settlement and customer-balance bank transfers can also remain asynchronous. The webhook verifies and classifies PaymentIntent lifecycle events, but durable automated fulfillment is not implemented; confirm `payment_intent.succeeded` in Stripe before beginning manual delivery.
